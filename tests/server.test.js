  ...
  describe('Delete /todos/:id route', () =>{
    it('should remove todo doc',(done)=>{
      ...
      .end((err, res)=> {
        ...
        Todo.findById(hexId).then((todo)=> {expect(todo).toBeFalsy();   //return null undefined
        }).catch((e) => {done(e)});
      });
      done();
    });
      ...
      it('should Not remove todo doc of other user',(done)=>{  //trying to delete user[1]'s todo by user[1]
        ...
        .end((err, res)=> {
          ...
          Todo.findById(hexId).then((todo)=> {expect(todo).toBeTruthy();
          }).catch((e) => {done(e)});
        });
        done();
      });
